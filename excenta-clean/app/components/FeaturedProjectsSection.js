...
function ProjectItem({ title, subtitle, image, origin }) {
	...
	const scale = useTransform(scrollYProgress, [0, 1], isMobile ? [1, 1] : [0.4, 1])
	const opacity = useTransform(scrollYProgress, [0, 1], [0, 1])
	const translateX = useTransform(scrollYProgress, [0, 1], isMobile ? [0, 0] : (origin === 'top left' ? [-100, 0] : [100, 0]))
	const translateY = useTransform(scrollYProgress, [0, 1], isMobile ? [0, 0] : [-100, 0])
